...
# List of ET sessions linked with age, Sex, etc. 
particList<-read_csv(file.path(wdir, 'data/partic_list.csv'))

# Concatenated output Output from pipeline in fractal-eye-analyses-MFDFA/
h <- read_csv(file.path(wdir, 'data/h_out.txt'))

# Concatenated Face-looking data 
aoi_data<- read_csv(file.path(wdir, 'data/face_out.txt'))

# Calibration precision spreadsheet  (from calver task & from DL task)
precision_info_calver <- read.csv(file = file.path(wdir, 'data', 'calver', 'calver_trials', '_output_reformatted.csv'),
                                  stringsAsFactors = FALSE)
precision_info_dl     <- read.csv(file = file.path(wdir, 'data', 'calver', 'dl_trials', '_output_reformatted.csv'),
                                  stringsAsFactors = FALSE)

# Log of cases that were skipped for some reason
log_calver <- read.csv(file = file.path(wdir, 'data', 'calver', 'calver_trials', '_summary.txt'), 
                       stringsAsFactors = FALSE, header = FALSE)
log_dl = read.csv(file = file.path(wdir, 'data', 'calver', 'dl_trials', '_summary.txt'),
                  stringsAsFactors=FALSE, header = FALSE)
```
...
# Are any sessions in particList *with DL files* not in the h output?
#################################################################
sessions <- particList %>% 
  filter(compareNA(1, DL_flag)) %>% 
  pull(et_collected)
...
# Who is in particList that does not have H data?
sessions[!(sessions %in% h_sessions)]

# Are any sessions with processed data that are not in particList?  
# Yes, but not an issue - the person is in particList, so the session # is prob mis-labeled 
#################################################################
h_sessions[!(h_sessions %in% sessions)]
...
```{r particListcsv, echo=FALSE, include=FALSE}
#Check if missing data in particList
particList %>%
  filter(is.na(Sex) | is.na(et_age) |is.na(DOB))%>% 
  # only care if it's missing if they have data
  filter(compareNA(1, DL_flag)) %>%
  select(CandID, PSCID, Sex, et_collected, et_age, cohort2, DL_flag, Cal_flag)
```
...
temp = particList %>%
  select(PSCID, CandID, Sex, n_et, cohort2, DOB, ageFirstVis) %>%
  distinct()
...
# Cases missing from particList (none)
missing_info = h %>%
  filter(particList==0) %>%
  filter(!grepl('bab', id, ignore.case=TRUE)) %>%
...
if (cleaned_precision_dat==0) {
  source(file.path(wdir, 'funcs', 'clean_calver.R'))
...
If visit does not have separate CalVer task, then use the trials in the DL File. 
```{r echo=TRUE}
calver_thresh
calver_dl_thresh
```
...
# If they have more than one value (did more than one calver task), use the better one 
AvgPrec_calver = AvgPrec_calver %>%
  select(-id_parts) %>%
  group_by(id) %>%
...
# If they have more than one value (did more than dl task), use the better one 
AvgPrec_dl = AvgPrec_dl %>%
  select(-id_parts) %>%
  group_by(id) %>%
  mutate(AvgPrecision = max(AvgPrecision)) %>%
...
```{r echo=FALSE}
# Check if the file was skipped in calver script (not enough data)
...
h_ids = h %>% 
  pull(id) %>%
  unique()
# Cases with no separate cal-ver file 
no_cal = setdiff(h_ids, cal_ids)
```


```{r}
# Cases that don't have calver data, but also are not in the DL-calver output (could be bc not enough data in attention trials for both files)
missing = no_cal[no_cal %in% dl_ids==FALSE] 

# Cases that have no calver data, and no eye-tracking files
...
This analysis included previously published data (Stallworthy et al., 2020), as well as data collected after the date of the last visit included in that manuscript (January 2018). This included `r as.integer(nrow(h))` time-series of eye-tracking data collected from 
`r h %>% pull(PSCID) %>% unique() %>% length() %>% as.integer()` `r min(h$et_age) %>% round(1)`-`r max(h$et_age) %>% round(1)`-month-old infants 
(`r h %>% select(PSCID,Sex) %>% distinct() %>% filter(Sex=='Female') %>% nrow() %>% as.integer()` females, 
mean age= `r h %>% select(id, et_age) %>% distinct() %>% pull(et_age) %>% mean() %>% round(1)` months) 
across `r h %>% pull(id) %>% unique() %>% length() %>% as.integer()` visits to the lab. As part of a planned missingness design, children contributed between `r min(waves$n)`  and `r max(waves$n)` waves of data across this age span (mean `r mean(waves$n)` waves) during visits to the lab.
...
```{r}
save(file = file.path(wdir, 'data', 'h_complete.RData'), h)
```
...
were linearly interpolated using data from the last valid sample before the start of the blink, and the first valid sample after the end of the blink <b>(mean proportion interpolated = `r round(mean(h_clean$propInterp),2)`, range `r round(min(h_clean$propInterp),3)`–`r round(max(h_clean$propInterp),3)`). </b> 
...
## Text for Supplement
<b>Quality-control exclusion criteria</b>. Infants’ eyes were calibrated to the eye-tracking equipment at the beginning of each visit, using the manufacturer’s five-point calibration procedure. Precision, or the distance between repeated samples of gaze points (cite), was included given that it is a measure of variability and thus is likely to influence DFA calculations that quantify nested patterns of variability. To measure the eye-tracker’s precision throughout the experiment, audio-visual attention cues were presented at the beginning of the experiment, and were interleaved with the Social and Pixelated movies. Infants’ longest contiguous raw eye-tracking data for all available Attention Cue trials were analyzed for precision. Data from eye-tracking sessions with Root Mean Square Error of Approximation (RMSEA) values two standard deviations or more above the sample mean (indicating poor precision) were excluded. As such, data from eye-tracking sessions with an average RMSEA (averaged across X and Y axes for both eyes) greater than <b>`r round(calver_thresh,2)`</b> degrees of visual angle were excluded from analyses. 
For the remaining eye-tracking visits, the average precision was <b>`r round(mean(h_clean$AvgPrecision), 2)`°</b> of visual angle.

Time series were also excluded if there was too much missing data that had to be linearly interpolated. After visualizing the distribution, the 80th quantile threshold for the sample’s proportion interpolated was calculated (`r round(interp_thresh[1], 2)`) and segments with a proportion exceeding that value were excluded from analyses.  After interpolation, infants had an average proportion of `r mean(resid_missing)` (range `r min(resid_missing)`–`r max(resid_missing)`) residual missing data per segment. Given past work suggesting too much aggregation can bias detrended fluctuation analysis (e.g., CITE20), we chose a stringent quality control threshold for the permissible proportion interpolated for each segment. 

The mean proportion of residual missing data for the remaining sample was `r round(mean(h_clean$propMissing), 3)`. 
Proportions of interpolated and residual missing data were entered into all models as covariates. 

## Manuscript text block
```{r echo=FALSE}
n_orig = nrow(h)
n_removed = sum(h$remove)
```

**Quality-control exclusion criteria.** Time series were excluded if an infant had poor eye-tracker calibration precision, if too much data were linearly interpolated when generating the time series for DFA, if the time series were too short (<800 frames, see Supplement for more information), and if the time series’ DFA yielded poor linear fit thereby suggesting unstable estimates of $\alpha$. Ultimately, we excluded `r n_removed`  time series for quality-control (`r round(n_removed/n_orig, 3)*100`% of the original `r n_orig` time series). The final sample included `r nrow(h_clean)` time series (a XX% increase from the sample we have previously published on). A flow diagram of the exclusionary process is shown in Figure XX. 
...
```{r}
exclusion_info = list(removed = removed, 
            PSCID_removed_calver=PSCID_removed_calver, 
            vis_removed_calver = vis_removed_calver,
            PSCID_removed_interp=PSCID_removed_interp,
            vis_removed_interp=vis_removed_interp,
            PSCID_removed_fix=PSCID_removed_fix,
            vis_removed_fix=vis_removed_fix, 
            PSCID_removed_r2=PSCID_removed_r2,
            vis_removed_r2=vis_removed_r2)

save(exclusion_info,file='data/exclusion_info.RData')
```

```{r}
qc_info = list(calver_thresh = calver_thresh, 
     calver_dl_thresh = calver_dl_thresh,
     interp_thresh = interp_thresh,
     resid_missing = resid_missing)

save(qc_info, file='data/qc_info.RData')
```
...
write.csv(h5, file.path(out_dir, paste(today, '-h_clean.csv', sep = '')))
write.csv(removed, file.path(out_dir, paste(today, '-h_removed.csv', sep = '')))
