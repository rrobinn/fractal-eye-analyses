...
# read participant info for chort 
partic_info = read.csv( paste(wdir, 'data/participantList.csv', sep = '') )
...
data$CalVer = as.factor(data$CalVer)
data$Pixelated = as.factor(data$Pixelated)
...
# Merge w. cohort info
data =data %>% left_join(partic_info, by = 'id')

# Re-name Center-calver
data=data%>%
  mutate(movie = as.character(movie)) %>%
  mutate(movie = ifelse(grepl('Center',movie), 'Center_converted.avi', movie) ) %>%
...
sm.density(data$h, xlab="H (Hurst Exponent) of Looking Patterns", model="normal")

qqnorm(data$h);qqline(data$h, col = 2)

skew=skewness(data$h) #btw -2 and 2 ideally
kurtosis=kurtosis(data$h) #3 for a normal distribution

mean(data$h) #0.84 (0.12-1.34)
data$pink=NA
data$pink=ifelse(data$h>0.69 & data$h<1.1, 1, 0)
sum(data$pink==1)/nrow(data)
```
