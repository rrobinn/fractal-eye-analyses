...
Are we missing cohort, DOB or sex for anyone in `indiv_dat`?
```{r}
indiv_dat %>%
...
missing = visit_dates %>%
  ...
  arrange(PSCID) %>% 
  select(PSCID) %>%
  mutate(fam_id = sapply(strsplit(missing, '_'), '[' ,1))
...
missing %>%
  mutate(real_id = ifelse(fam_id %in% indiv_dat$fam_id, 1, 0))
