...
# Make variables for avg face looking grouped by condition
...
model_dat %>%
  ...
  dplyr::select(Participant, Visit, movie, seg, longestFixDur, h)
...
mynames <- as.character (c("glm.0", "glm.1", "glm.1b")) # model numbers as characters
myaicc  <- ICtab(logLik(glm.0), logLik(glm.1), logLik(glm.1b),
		     type ="AICc",
		     nobs = nrow(na.omit(model_dat)),
		     weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc$rweight <- max(myaicc$weight)/myaicc$weight
