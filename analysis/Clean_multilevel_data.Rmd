...
# Fussed out 
# # # # # # # # # 
# List of time series that survived up to & including current variable 
survived = h %>%
  filter(!grepl('Age|Fuss', reason)) %>%
  select(id, PSCID,reason) %>%
  distinct()

removed_fuss = removed %>%
  filter(reason=='Fussed out') %>% 
  select(id, PSCID) %>% 
  distinct() %>%
  mutate(removed_PSCID_from_sample = ifelse(PSCID %in% survived$PSCID, 0, 1))

PSCID_removed_fuss = removed_fuss %>% filter(removed_PSCID_from_sample==1) %>% pull(PSCID) %>% unique()
vis_removed_fuss = removed_fuss %>% pull(id) %>% unique()
```
Data from  <b>`r sum(removed_fuss$removed_PSCID_from_sample)` participants, `r length(unique(removed_fuss$id))` eye-tracking visits, totaling `r removed %>% filter(grepl('Fussed', reason)) %>% nrow()` </b> time-series were excluded from future analyses.
...
survived = h %>%
  ...
  filter(!grepl('Age|Fuss|Cal', reason)) %>%
  select(id, PSCID,reason) %>%
  distinct()
...
intersect(PSCID_removed_calver, PSCID_removed_fuss)
intersect(vis_removed_calver, vis_removed_fuss)
...
As a result, data from <b>`r sum(removed_calver$removed_PSCID_from_sample)` participants, `r length(unique(removed_calver$id))` eye-tracking visits, totaling `r removed %>% filter(grepl('Calver', reason)) %>% nrow()` </b> time-series were excluded from future analyses. For the remaining eye-tracking visits, the average precision was <b>`r mean(h_clean$AvgPrecision)`°</b> of visual angle.
...
# Make sure these were not included in kids already removed
intersect(PSCID_removed_interp, c(PSCID_removed_fuss, PSCID_removed_calver))
intersect(vis_removed_interp, c(vis_removed_fuss,vis_removed_calver))
```
...
`r removed_interp %>% select(id,removed_PSCID_from_sample) %>% distinct() %>% pull(removed_PSCID_from_sample) %>% sum()` infants, 
`r removed_interp %>% select(id,removed_vis_from_sample) %>% distinct() %>% pull(removed_vis_from_sample) %>% sum()`  eye-tracking session, 
...
survived = h %>%
  # Get list of Ids that are still there, after you remove for all the resons leading up to & including propInterp
  filter(!grepl('Age|Fuss|Calver|propInterp|Fix', reason)) %>%
  select(id, PSCID,reason) %>%
  distinct()
removed_fix = removed %>%
...
# Make sure these were not included in kids already removed
intersect(PSCID_removed_fix, c(PSCID_removed_fuss, PSCID_removed_calver,PSCID_removed_interp))
intersect(vis_removed_fix, c(vis_removed_fuss,vis_removed_calver,vis_removed_interp))
```
...
survived = h %>%
  # Get list of Ids that are still there, after you remove for all the resons leading up to & including propInterp
  filter(!grepl('Age|Fuss|Calver|propInterp|Fix|R2', reason)) %>%
  select(id, PSCID,reason) %>%
  distinct()
...
intersect(PSCID_removed_r2, 
          c(PSCID_removed_fuss, PSCID_removed_calver,PSCID_removed_interp,PSCID_removed_fix))

intersect(vis_removed_r2, 
          c(vis_removed_fuss,vis_removed_calver,vis_removed_interp, vis_removed_fix))
```
...
lab1 = paste('Original Sample \n', 
       'N = ', length(unique(h$PSCID)), ' (', n_females, ' females) \n',
      'Visits = ', length(unique(h$id)), '\n',
      'Time-series = ', nrow(h), sep='')

lab_fuss = paste('Excluded for fussing out \n', 
      'N = ', length(PSCID_removed_fuss), '\n', 
      'Visits = ', length(vis_removed_fuss), '\n',
      'Time-series = ', removed %>% filter(grepl('Fussed', reason)) %>% nrow(), sep='')

lab_cal = paste('Excluded for low eye-tracker precision \n', 
       'N = ', length(PSCID_removed_calver), '\n',
...
h2=left_join(h %>% filter(remove==0), 
