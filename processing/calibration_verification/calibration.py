...
## Last updated  Feb 2021 by Robin D Sifre <robinsifre@gmail.com>
"""
  ...
  for eachstimulus in l_dur:
    for eachfixation in l_dur[eachstimulus]:
      ...
      try:
        ...
        tempave = "N/A"
        print("Couldn't get average points for stimulus %s / fix %s, no values found." %(eachstimulus, eachfixation))
...
\n\tone eye was always a negative value.\n" %(filename.split('/')[-1], eachfixation))
    ...
    # [1445, 283], [1454, 277], [1430, 268]], [1443.0869565217392, 275.95652173913044]]},
      ...
      if verbose:
        print("Finding Euclidean distance from average points to stimulus coordinates for %s / %s..." %(eachstimulus, eachfixation))
    # using the average points found above, find the Euclidean distance from average to actual stimulus points.
      try: 
        ...
        temppoints = "N/A"
        print("Couldn't get Euclidean distance points for stimulus %s / %s, no values found." %(eachstimulus, eachfixation))
  ...
  for eachstim in l_dur.keys():
    ...
    for eachfix in l_dur[eachstim].keys():
      if eachfix == '-1':
        ...
        if euclid_dist >= 6.:
          print("Stimulus %s, fixation #%s, degrees is %f >= 6, so removing from consideration." %(eachstim, eachfix, euclid_dist))
  ...
  for eachstim in l_dur.keys():
    ...
    for curfix in l_dur[eachstim].keys():
      ...
      # Are you tied? Keep the CLOSET fixation by degree.
        ...
        print("There was a tie for longest fixation.  Old index %s / time %s / degree %s; new fix %s / time %s / degree %s." %(longestFixIndex, longestFixValue, longestFix_euclid_dist, curfix, l_dur[eachstim][curfix][2], curfix_euclid_dist))
