...
# FOr when you have DOB from database 
# h = h %>%
...
h2%>%
  group_by(Participant)%>%
  dplyr::select(faceCount, otherCount) %>%
  summarise(AvePropFace_person = sum(faceCount, na.rm = TRUE) / ( sum(faceCount, na.rm = TRUE) + sum(otherCount, na.rm = TRUE) ),
            n_person=n())
