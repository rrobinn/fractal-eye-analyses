...
scale0=round(2.^exponents); %creates a small scale 

%Computing of regression line for q=0 (see Matlab Code 8)
for ns=1:length(scale0), %loop for each segement size to compute local fluctuations
    segments(ns)=floor(length(X)/scale0(ns));
    for v=1:segments(ns),
		Index0=((((v-1)*scale0(ns))+1):(v*scale0(ns)));
		C0=polyfit(Index0,X(Index0),m);
		fit0=polyval(C0,Index0);
		RMS0{ns}(v)=sqrt(mean((X(Index0)-fit0).^2));%local fluctuations
    end
    Fq0(ns)=exp(0.5*mean(log(RMS0{ns}.^2)));
end

%Estimation of local fluctuations, centered at v instead of within
%non-overlapping segments 
halfmax=floor(max(scale)/2); %number of samples at start and end of time-series, excluded
Time_index=halfmax+1:length(X)-halfmax; %point to which to align center of segments
...
%Estimation of local Hurst expontent (Ht) from local fluctuations
C = polyfit(log2(scale0),log2(Fq0),1); %defines regression line
Regfit = polyval(C,log2(scale)); %represents center of spread of local RMS
Hq0=C(1);
maxL=length(Time_index);
for ns=1:length(scale); %computes residual fluctuation
	RMSt=RMS{ns}(Time_index);
	resRMS=Regfit(ns)-log2(RMSt); %residual fluctuation of log2(RMS{ns})
	logscale=log2(maxL)-log2(scale(ns));
    Ht(ns,:)=resRMS./logscale + Hq0; %local Hurst exponent
end

Ht_row=Ht(:);
BinNumb= round(sqrt(length(Ht_row)));
%creates number of bins for the histogram
[freq,Htbin]=hist(Ht_row,BinNumb);
Ph=freq./sum(freq); %dividing number frequency of local Hurst exponents in each bin by total number of local Hurst exponents
Ph_norm=Ph./max(Ph);
%log-transforming the normalized probability distribution of Ph
