...
% Code written by Espen Ihlen  https://www.mathworks.com/matlabcentral/fileexchange/38262-multifractal-detrended-fluctuation-analyses
...
%% % STEP 1. Data prep
warning off
X=cumsum(signal-mean(signal)); %integrating mean centered data
...
%% % STEP 2. Multifractal Detrended Fluctuation Analysis

%calculating local RMS and q-order RMS's to account for different weighting
for ns=1:length(scale),%looping through length of scale
    ...
    for v=1:segments(ns), %loop computes local RMS around a trend fit {v} for each segment
        ...
        C=polyfit(Index,X(Index),m); %creates polynomial coeffcient from m
        fit=polyval(C,Index); %fits polynomial trend to each segment
        RMS_scale{ns}(v)=sqrt(mean((X(Index)-fit).^2)); %local fluctuation of residual variation
    end
    for nq=1:length(q), %computes overall q-order RMS
        qRMS{nq,ns}=RMS_scale{ns}.^q(nq);
        Fq(nq,ns)=mean(qRMS{nq,ns}).^(1/q(nq)); %q-order RMS
    end
    Fq(q==0,ns)=exp(0.5*mean(log(RMS_scale{ns}.^2))); %redefines q=0 to avoid infinity
end
%defines q-order Hurst exponents, slopes (Hq) of regression lines for each q-order RMS
for nq=1:length(q),
...
%% % STEP 3. Multifractal Spectrum

%q-order mass exponent, tq, computed from Hq
tq = Hq.*q-1; %mass exponent
%mass exponent, tq, used to compute q-order singularity exponent, hq
hq = diff(tq)./(q(2)-q(1)); %q-order singularity exponent
%mass exponent tq is used to compute q-order singularity exponent (hq) and q-order singularity dimension (Dq)
Dq = (q(1:end-1).*hq)-tq(1:end-1); %q-order singularity dimension; spectrum width!

%% % STEP 4. Plot summary figure (optional)
%OUTPUT FIGURE-------------------------------------------------------------
if Fig==1,
