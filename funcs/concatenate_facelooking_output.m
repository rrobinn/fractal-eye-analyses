...
header = 'id, movie, seg, date, longestFixDur, propMissing, propInterpolated, faceCount, otherCount'; 
if nargin>1
       ...
       % Read in .mat file w/ segmented data
        ...
        % Get rid of empty cells 
         ...
         for s = 1:size(segmentedData,1)
             ...
             propInterpolated = temp{1, segSummaryCol.propInterpolated};
             propMissing = temp{1, segSummaryCol.propMissing};
             ...
             temp_out = [session ',' trial ',' num2str(seg) ',' ...
                 date, ',', num2str(longestFix) ',' , ...
                 num2str(propMissing), ',', num2str(propInterpolated), ','...
                 num2str(faceCount) ',' num2str(otherCount)];
             fprintf(fid, '%s \n', temp_out);
         end
   catch ME
