...
To determine with ET visits have usable rs-fMRI data, the following info is pulled:
```{r echo = TRUE}
# List of directories w/ RS .pconn files
pconn_file = '~/Documents/GitHub/enrichment-repetitive-bx/Marginal-Model-Data/listofpconns.txt'
# Visits w/ enough ET data
min_data_check_file = paste(wdir, "data/misc_queries/visits_with_atleast_2ts.csv", sep='')
```
...
old_data = h2 %>%
  filter(excl1 ==0) %>%
  dplyr::select(session = id, Cohort = cohort, PSCID = Participant, age, visit_date=date) %>%
  distinct()
```
...
vis_list = rbind(old_data, 
                 visits %>% dplyr::select(session, Cohort, PSCID, visit_date, Cohort, Cohort2, age, old))
...
```{r}
# Determine which eye-tracking visits have scans
visit = c()
scan_name = c()
has_scan = c()
# For every visit in list of eye-tracking visits 
for (s in vis_list2$session) {
  a=vis_list2 %>% filter(session == s)
  id = a$CandID
  age=a$age
  
  visit = c(visit, a$session)
  if (id %in% listofpconns$CandID) {
    
    # Determine if there is a usable RS 
    temp = listofpconns %>% 
      filter(CandID == id) %>%
      mutate(age_diff = abs(vis - age)) %>%
      filter(age_diff == min(age_diff))
    
    s_name = temp$fname[1]
    
    closest_match = temp %>% 
      # Some filenames are slightly different (e.g. 9m and 9mo) but it's the same visist
      dplyr::select(exists,CandID, vis, age_diff) %>%
      distinct()
    
    if (nrow(closest_match)>1) { # only the case for one 
      closest_match = closest_match[1,]
    }
    
    if (closest_match$age_diff <= 1){
      scan_name = c(scan_name, s_name)
      has_scan = c(has_scan, closest_match$exists)
    } else{
      scan_name = c(scan_name, 'None')
      has_scan = c(has_scan, 'missing')
    }
...
scan_info = data.frame(session = visit, has_scan=has_scan, scan_name = scan_name)
```

```{r}
# Merge scan info
vis_list2=left_join(vis_list2, scan_info, by = 'session')
```

Eye-tracking visits <b>with</b> RS data
```{r}
datatable(vis_list2 %>% 
            filter(has_scan=='exists') %>%
            dplyr::select(session, Cohort2, age, CandID, scan_name),
          filter='top'
          )

```

Eye-tracking visits <b>without</b> RS data
```{r}
datatable(vis_list2 %>% 
            filter(has_scan=='missing') %>%
            dplyr::select(session, Cohort2, age, CandID, scan_name),
          filter = 'top', options = list(digits=2)
          )
```

## Visualize visits with eye-tracking & RS data
```{r bslerp_et_rs_fig}
vis_list2 %>%
  filter(Cohort2=='BSLERP') %>%
  mutate(has_scan = factor(has_scan, c('missing', 'exists'))) %>%
  ggplot(data = ., aes(y = reorder(PSCID, ageFirstVis), x = age)) +
  geom_point(aes(color = has_scan), size=1) +
  geom_line() +
  theme(axis.text.y=element_blank()) + 
  scale_y_discrete()+
  labs(x = 'Dancing Ladies Age (months)', y = 'Individual', title='BSLERP') 
...
```{r bcpC_bcpD_et_rs_fig}
vis_list2 %>%
  filter(Cohort2=='BCP_C' | Cohort2=='BCP_D' | Cohort2 == 'BCP_E' | Cohort2=='BCP_ELISON') %>%
  mutate(has_scan = factor(has_scan, c('missing', 'exists'))) %>%
  ggplot(data = ., aes(y = reorder(PSCID, ageFirstVis), x = age)) +
  geom_point(aes(color = has_scan), size=1) +
  geom_line() +
...
Kids w/ no RS data for any visit (all BSLERP & DevSocAtt kids)
```{r}
# Determine who has no RS data 
temp = vis_list2 %>%
  group_by(PSCID) %>%
  summarise(visit_count = n(),
            noscan_count = sum(has_scan=='missing'),
            no_RS = ifelse(visit_count == noscan_count, 1, 0)) 

vis_list2=left_join(vis_list2, temp %>% dplyr::select(PSCID, noscan_count, no_RS), by = 'PSCID')
...
datatable(vis_list2 %>%
            filter(no_RS==1) %>%
            dplyr::select(PSCID, Cohort,Cohort2, n_sess) %>%
            distinct(),
           filter= 'top')
