...
```{r}
particList %>%
  filter(is.na(DOB) | is.na(Sex) | is.na(et_age))
```
...
## Bin the cohorts (to do)
```{r}

```


## Check is missing participant level data once we merge  
Missing DOB 
```{r}
h %>% filter(is.na(DOB)) %>% select(Participant, id, DOB, et_age, et_date, cohort2) %>% distinct()
```
