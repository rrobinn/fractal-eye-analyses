...
dat <- read_csv("/Users/sifre002/Box/sifre002/7_MatFiles/01_Complexity/stability-experiments/parameter-stability/stability_out.txt")
# Read calver data
...
# Remove sessions w/ bad calver & where h was not calculated
good_calver_id = AvgPrecision %>%
  filter(AvgPrecision!=999)%>%
  select(id) 
dat2 = dat %>%
  filter(id %in% good_calver_id$id) %>%
...
# Convert parameters to factors for easier plotting
# dat2 = dat2 %>%
#   mutate(scres=as.factor(scres),
#          scmin=as.factor(scmin),
#          scmaxdiv=as.factor(scmaxdiv))
