...
# To do
- Check why there is no surrogate data for JE000500_03_04,  JE000221_05_07
```{r echo=FALSE}
...
h = read.csv( paste(wdir, 'data/2021-04-01-h_clean.csv', sep = ''), stringsAsFactors = FALSE )
...
```{r}
h = h %>%
  mutate(orig_width_posonly = pmax(Hq0, Hq_Pos1, Hq_Pos3, Hq_Pos5) - 
           pmin(Hq0, Hq_Pos1, Hq_Pos3, Hq_Pos5),
        orig_width_allqs   = pmax(Hq_Neg5, Hq_Neg3, Hq_Neg1, Hq0, Hq_Pos1, Hq_Pos3, Hq_Pos5) - 
...
h %>%
  dplyr::select(orig_width_posonly, orig_width_allqs) %>%
  pivot_longer(cols = c(orig_width_posonly, orig_width_allqs),
...
surr = surr %>%
  ...
  mutate(# Including only positive q's 
         min_hq_posonly = pmin(`Hq0`, `Hq1`, `Hq3`, `Hq5`),
         max_hq_posonly = pmax(`Hq0`, `Hq1`, `Hq3`, `Hq5`),
         surr_width_posonly = max_hq_posonly - min_hq_posonly,
         # Including all q's
         min_hq = pmin(`Hq.5`, `Hq.3`, `Hq.1`, `Hq0`, `Hq1`, `Hq3`, `Hq5`),
         max_hq = pmax(`Hq.5`, `Hq.3`, `Hq.1`, `Hq0`, `Hq1`, `Hq3`, `Hq5`),
         surr_width = max_hq - min_hq) %>%
  dplyr::select(-c(min_hq_posonly, max_hq_posonly, min_hq, max_hq))
```
...
surr_summ = surr %>%
  ...
  summarise(
            ...
            surr_mean_width_posonly = mean(surr_width_posonly),
            surr_min_width_posonly = min(surr_width_posonly),
            surr_max_width_posonly = max(surr_width_posonly),
            surr_sd_width_posonly = sd(surr_width_posonly)) %>%
  ungroup()

# Make wide (each time series is its own row. Populate the surrogate columns with spectrum widths
surr_wide = surr %>%
  dplyr::select(temp, surrogate_num, surr_width, surr_width_posonly) %>%
  pivot_wider(id_cols = temp, 
              names_from=surrogate_num, names_prefix = 'surr', 
              values_from = c(surr_width, surr_width_posonly))
...
temp = h %>%
  dplyr::select(temp,
                surr_mean_width_posonly,
                surr_mean_width,
                orig_width_posonly,
                orig_width_allqs,
                Trial) %>%
  pivot_longer(cols = c(surr_mean_width_posonly, surr_mean_width,
                        orig_width_posonly, orig_width_allqs),
               names_to='data_source', values_to='width')
```


```{r echo=FALSE}
# temp %>% filter(data_source=='surr_mean_width_posonly') %>% dplyr::select(width) %>% summary(width) 
# temp %>% filter(data_source=='orig_width_posonly') %>% dplyr::select(width) %>% summary(width) 
# 
# temp %>% filter(data_source=='surr_mean_width') %>% dplyr::select(width) %>% summary(width) 
# temp %>% filter(data_source=='orig_width_allqs') %>% dplyr::select(width) %>% summary(width) 
```
...
- When we calculated MF spectra width with <b>positive q's only</b>, the surrogates are a bit wider than the original data. 
```{r orig_width_surrogate_width_hist, echo=FALSE, warning=FALSE, cache=TRUE}
temp = temp %>% mutate(posonly = ifelse(grepl('pos',data_source), 1, 0))
...
ggplot(data=., aes(x=width, fill=data_source, color=data_source)) +
  ...
  labs(caption= 'Spectrum widths computed with only positive q\'s', x = 'MF spectrum width')

temp %>%
  filter(width <= 2.5,
         grepl('pos', data_source)) %>%
  mutate(data_source = ifelse(grepl('surr',data_source), 'Surrogate Mean Width', 'Original Time Series Width')) %>%
ggplot(data=., aes(x=data_source, y=width, color=data_source)) +
  geom_boxplot() + 
  theme_bw() +
  theme(legend.title = element_blank()) + 
  labs(caption= 'Spectrum widths computed with only positive q\'s', 
       x = '', y= 'MF spectrum width')

```


```{r origwidth_vs_surrwidth_posonly, echo=FALSE, cache=TRUE}
ggplot(data = h, aes(x=orig_width_posonly, y =surr_mean_width_posonly, color=Trial )) +
  geom_point(alpha=0.2) + 
  geom_abline(slope=1)+
  facet_wrap(~Trial, nrow=3) + 
  labs(x= 'Spectrum Width (Original data)', y = 'Surrogate mean width',
       title = 'Spectrum widths computed with only positive q\'s')
```

- When we calculate MF spectra with <b>negative and positive q's</b>, the original time series had a fatter tail than the surrogats.  
...
ggplot(data=., aes(x=width, fill=data_source, color=data_source)) +
  geom_histogram(alpha=0.2, position='identity')+ 
  labs(title= 'Spectrum widths computed with positive and negative q\'s')
```
...
ggplot(data=., aes(x=data_source, y=width, color=data_source)) +
  ...
  theme_bw() +
  theme(legend.title = element_blank()) + 
  labs(caption= 'Spectrum widths computed with all q\'s', 
       x = '', y= 'MF spectrum width')
```
...
```{r origwidth_vs_surrwidth_allqs, echo = FALSE, cache=TRUE}
h %>% 
  filter(orig_width_allqs <= 2.5) %>%
ggplot(data = ., aes(x=orig_width_allqs, y =surr_mean_width, color=Trial )) +
  geom_point(alpha=0.2) + 
  geom_abline(slope=1)+
  facet_wrap(~Trial, nrow=3) + 
  labs(x= 'Spectrum Width (Original data)', y = 'Surrogate mean width',
       caption = 'Spectrum widths computed with positive and negative q\'s')
```
...
my_test <- function(dat) {
  ...
  x_all =c(dat$surr_width_surr1, dat$surr_width_surr2, dat$surr_width_surr3, dat$surr_width_surr4,
              dat$surr_width_surr5, dat$surr_width_surr6, dat$surr_width_surr7, dat$surr_width_surr8)
  x_posonly = c(dat$surr_width_posonly_surr1, dat$surr_width_posonly_surr2, dat$surr_width_posonly_surr3,
                dat$surr_width_posonly_surr4, dat$surr_width_posonly_surr5, dat$surr_width_posonly_surr6,
                dat$surr_width_posonly_surr7, dat$surr_width_posonly_surr8)
  
  # Pull the original data surrogate width 
  mu_all = dat$orig_width_allqs
  mu_posonly = dat$orig_width_posonly
  
  # Check if there is data 
  if (sum(!is.na(x_all))<3) {return(list(t=NA, p=NA))}
  if (sum(!is.na(x_posonly))<3) {return(list(t=NA, p=NA))}

  # Do the t-test 
  ttest_all = t.test(x_all, mu=mu_all ) # alternative hypothesis: true mean of distribution is outside of  mu/spect_width
  ttest_posonly = t.test(x_posonly, mu=mu_posonly ) 

  return(list(t_all = ttest_all$statistic, p_all = ttest_all$p.value,
              t_pos = ttest_posonly$statistic, p_pos = ttest_posonly$p.value))
}
...
h = h %>%
  ...
  do({
    ...
    result$t_all = my_test(result)$t_all
    result$p_all = my_test(result)$p_all
    result$t_pos = my_test(result)$t_pos
    result$p_pos = my_test(result)$p_pos
    result
  })

save(h, file = paste(wdir, 'data/h_with_tstats.RData',sep=''))

```

## Remove extreme outliers 
```{r, cache=TRUE}
quant_all = quantile(h$t_all, na.rm=TRUE, c(.01, .99))
quant_pos = quantile(h$t_pos, na.rm=TRUE, c(.01,.99))

h2 = h %>%
  mutate(t_all_outlier = ifelse(t_all < quant_all[['1%']] | t_all > quant_all[['99%']], 1, 0),
         t_pos_outlier = ifelse(t_pos < quant_pos[['1%']] | t_pos > quant_pos[['99%']], 1, 0))
```

Example of data (first 500 rows)
```{r, cache=TRUE}
h2 %>%
  dplyr::select(PSCID, visit=id, movie, seg, Trial,
                orig_width_posonly, orig_width_allqs, 
                surr_min_width_posonly, surr_mean_width,
                t_all, t_pos) %>%
  head(500) %>%
  datatable(filter='top') %>%
  formatRound(columns = c('orig_width_posonly', 'orig_width_allqs', 'surr_min_width_posonly', 'surr_mean_width', 't_all', 't_pos'), digits=3)
...
```{r}
summary(h2$t_all)
summary(h2 %>% filter(t_all_outlier==0) %>% pull(t_all))

summary(h2$t_pos)
summary(h2 %>% filter(t_pos_outlier==0) %>% pull(t_pos))
