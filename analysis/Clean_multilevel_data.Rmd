...
Participants in `h_out.txt` not in the participant list. 
```{r}
# Log who was removed 
h = h %>%
  mutate(remove = ifelse(particList==0, 1, 0),
         reason = 'Not in participant list')
```
...
```{r}
h = h %>% 
  mutate(remove = ifelse(et_age<min_age | et_age>max_age, 1, remove),
         reason = ifelse(et_age<min_age | et_age>max_age, 1, 'Age Range'))
...
```{r}
h = h %>% 
  mutate( remove_precision = ifelse(is.na(Precision_RMS_X_Y),1,0) )
